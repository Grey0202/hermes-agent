...
class CredentialPool:
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        if entry.auth_type != AUTH_TYPE_OAUTH or not entry.refresh_token:
            ...
            # here (STATUS_EXHAUSTED, 1h TTL) is wrong: the status persists to
            # the SHARED auth.json and poisons every gateway and cron until a
            # full restart. Hydrate from the credentials file first, and only
            # mark exhausted if that yields nothing usable.
            # Adapted from upstream PR #58096 (fork-local patch 028).
            if (
